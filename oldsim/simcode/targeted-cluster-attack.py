...
def run_env(A, B, H, signs):
    ...
    return [sum(diff_A_B), sum(diff_H_A), sum(diff_H_B)]

def sign(x):
    return 1 if x > 0.5 else 0


def remove_edges(g, n = 1, cluster=False):
    for i in xrange(n):
        edgelist = g.get_edgelist()
        if edgelist:
            if cluster:
...
def run_attack_diff_sim(G):
    ...
    for j in xrange(20):
        rewire(B)
    H = hybrid(A, B)
    res = (resAB, resHA, resHB) = run_env(A, B, H, signs)
    return res
...
def run_many(G, num_changes = 10):
    ...
    for i in xrange(num_changes):
        (resAB, resHA, resHB) = run_attack_diff_sim(G)
        diff_AB.append(resAB)
        diff_HA.append(resHA)
        diff_HB.append(resHB)
        remove_edges(G, n=3, cluster=True)
    return (diff_AB, diff_HA, diff_HB)
...
def main():
    ...
    (diffAB, diffHA, diffHB) = y = run_many(G, num_changes=10)
    if len(sys.argv)>1:
        # save to file
        f = open(sys.argv[1], 'w')
